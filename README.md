...
## Amélioration : factorisation

Si un même calcul coûteux est fait plusieurs fois, 
stocker le résultat dans une variable plutôt qu'effectuer le calcul plusieurs fois.
```java
float compute(float angle) {
...
## Amélioration : cohérence des types
...
## Amélioration : valeurs approchées si précision acceptable
...
Si le résultat ne nécessite pas une grande précision.
Il peut être acceptable d'utiliser les méthodes approximatives moins coûteuses
...
## Limiter le nombre de librairies importées
...
## Mesurer avec Java Flight Recorder
...
![Flame graph sur CPU time](assets/jfr_cpu_time_flame_graph.png)

Quand on a analysé dans le code depuis intelliJ on peut même voir des infos directement dans le code.

![CPU time depuis le code](assets/jfr_cpu_time_in_code.png)

### Memory allocations

![Allocations mémoire triées par leur propre allocation](jfr_memory_allocations_sorted_by_own_allocations.png)


![img.png](assets/jfr_memory_allocations_in_code.png)
